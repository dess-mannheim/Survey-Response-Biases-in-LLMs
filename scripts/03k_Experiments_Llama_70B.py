  ...
  for row in range(0,len(questionnaire)):
    ...
    for i,k in enumerate(bias_answer_columns):
        ...
        if count % 1000 == 0:
            ...
            model_response["model_name"] = "Llama-3.3-70B-Instruct"
            current_date = get_date_string()
            model_response.to_csv(f"../assets/final_interviews/{current_date}_Llama-3.3-70B-Instruct_selected_interviews.csv", encoding="utf-8", sep=";")
            print("1.000 batch appended")
